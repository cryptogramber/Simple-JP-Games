function TenkoGame(TenkoInput, TenkoStorage) {
	...
	this.tenkoInput.on("switchPage", this.switchPage.bind(this));
...
TenkoGame.prototype.switchPage = function(which_page) {
	switch(which_page) {
	case 0:	// help
...
TenkoGame.prototype.addQA = function() {
    ...
    var currentQ = (Math.floor(Math.random() * (self.tc.level_index_max - self.tc.level_index_min)) + self.tc.level_index_min);
...
TenkoGame.prototype.reverseMode = function(move_choice) {
	...
	if (self.contentDirection == 0) {
		...
		self.reverse_update.textContent = "あ";
	};
